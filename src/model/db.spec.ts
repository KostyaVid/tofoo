...
import { ISprint, Sprint } from './sprints';
...
describe('DataBase ', () => {
  ...
  describe('DataBase User', () => {
    it('Create new user', async () => {
      ...
      const sqlQuery = `SELECT * FROM user WHERE email = '${escape('testemail@google.com')}'`;
      const [user] = await db.get(sqlQuery);
      expect(user.username).toEqual('test');
      await db.delete(`DELETE FROM user WHERE email='${'testemail@google.com'}'`);
    });
  ...
  describe('DataBase sprint', () => {
    it('Create new sprint', async () => {
      ...
      await db.delete(`DELETE FROM sprint WHERE sprint_id=${sprint?.sprint_id}`);
    });
  });
  ...
  describe('DataBase project', () => {
    it('Create new project', async () => {
      ...
      await db.delete(`DELETE FROM project WHERE project_id=${project?.project_id}`);
    });
  });
